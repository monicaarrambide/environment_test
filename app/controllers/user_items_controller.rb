class UserItemsController < ApplicationController
  before_action :set_user_item, only: %i[ show edit update destroy ]

  # GET /user_items or /user_items.json
  def index
  ...
  def create
    ...
    respond_to do |format|
      if @user_item.save
        format.html { redirect_to user_item_url(@user_item), notice: "User item was successfully created." }
  ...
  def update
    respond_to do |format|
      if @user_item.update(user_item_params)
        format.html { redirect_to user_item_url(@user_item), notice: "User item was successfully updated." }
  ...
  # DELETE /user_items/1 or /user_items/1.json
  def destroy
    @user_item.destroy
